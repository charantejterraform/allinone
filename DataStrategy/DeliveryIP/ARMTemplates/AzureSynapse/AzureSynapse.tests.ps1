...
Describe "Template: Azure Synapse Tests" -Tags Unit {
    BeforeAll {
        #Setup
        $file = (get-location).Path + '\AzureSynapse.json'
        $devParameterFile = (get-location).Path + '\Parameters\Dev-AzureSynapse.parameters.json'
        $TemplateFileName = "AzureSynapse.json"       
