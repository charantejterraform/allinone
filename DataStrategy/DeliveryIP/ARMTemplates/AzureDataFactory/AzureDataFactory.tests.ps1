<#
	.NOTES
		...
		File:		AzureDataFactory.tests.ps1

		Purpose:	Pester - Test ARM Template and Parameter Files

		Version: 	1.0.0 - 3rd July 2021 - Microsoft Services
		==============================================================================================
...
param (
	...
	[Parameter()][string]$script:TemplateFileName = 'AzureDataFactory.json'
)
...
Describe "Template: $(Split-Path $($(Get-Item $PSScriptRoot).FullName) -Leaf)" -Tags Unit {
	...
	Context "Template File Syntax" {
		...
		It " Creates the expected Azure resources" {
            $expectedResources = @( 'Microsoft.DataFactory/factories'
            )
