...
Describe "Template: Azure Synapse Tests" -Tags Unit {
    BeforeAll {
        #Setup
        $file = (get-location).Path + '\AzureKeyVault.json'
        $devParameterFile = (get-location).Path + '\Parameters\Dev-AzureKeyVault.parameters.json'
        $TemplateFileName = "AzureKeyVault.json"       
    ...
    Context "Template Syntax" {       
        ...
        It "Converts from JSON and has the expected properties" {
            $expectedProperties = @('contentVersion',
                'parameters',
                'variables',
                'resources',
                'outputs'
